...
def compute_indicators(df):
    ...
    stoch_rsi_raw     = (df["RSI"] - rsi_min) / (rsi_max - rsi_min + 1e-10)
    df["StochRSI_K"]  = stoch_rsi_raw.rolling(3).mean() * 100
...
with st.sidebar:
    ...
    if page == "📊 Technical Analysis":
        st.markdown("### Chart")
        ta_range    = st.radio("Period", ["1W", "3W", "6W", "3M"], index=0, horizontal=True)
        show_bb       = st.checkbox("Bollinger Bands",  value=True)
        show_rsi      = st.checkbox("RSI (14)",          value=True)
        show_stochrsi = st.checkbox("Stochastic RSI",    value=False)
...
if page == "📊 Technical Analysis":
    ...
    if show_stochrsi:
        row_specs.append([{}]); row_heights.append(0.16); subtitles.append("Stochastic RSI")
    if show_macd:
        row_specs.append([{}]); row_heights.append(0.18); subtitles.append("MACD")
    if show_atr and "ATR_pct" in df_ta.columns:
        row_specs.append([{}]); row_heights.append(0.14); subtitles.append("ATR %")
    ...
    if show_stochrsi and "StochRSI_K" in df_ta.columns:
        fig_ta.add_trace(go.Scatter(x=df_ta.index, y=df_ta["StochRSI_K"].squeeze(),
            name="StochRSI %K", line=dict(color="#f78166", width=1.5)), row=cur_row, col=1)
        fig_ta.add_trace(go.Scatter(x=df_ta.index, y=df_ta["StochRSI_D"].squeeze(),
            name="StochRSI %D", line=dict(color="#79c0ff", width=1.5)), row=cur_row, col=1)
        fig_ta.add_hline(y=80, line_dash="dash", line_color="#f85149", opacity=0.4,
                         row=cur_row, col=1)
        fig_ta.add_hline(y=20, line_dash="dash", line_color="#3fb950", opacity=0.4,
                         row=cur_row, col=1)
        fig_ta.update_yaxes(range=[0, 100], row=cur_row, col=1)
        cur_row += 1
    ...
    if show_atr and "ATR_pct" in df_ta.columns:
        fig_ta.add_trace(go.Scatter(x=df_ta.index, y=df_ta["ATR_pct"].squeeze(),
            name="ATR %", line=dict(color="#d29922", width=1.5),
            fill="tozeroy", fillcolor="rgba(210,153,34,0.08)"), row=cur_row, col=1)
        fig_ta.update_yaxes(ticksuffix="%", row=cur_row, col=1)
        cur_row += 1
    ...
    with st.expander("📋 Signal Summary"):
        ...
        # Composite score: count bullish signals
        score_pts, score_max = 0, 0
        st.markdown("**RSI**")
        score_max += 1
        if rsi_now > 70:
            st.error(f"RSI {rsi_now:.1f} – Overbought")
        elif rsi_now < 30:
            st.success(f"RSI {rsi_now:.1f} – Oversold (potential reversal)")
            score_pts += 1
        else:
            st.info(f"RSI {rsi_now:.1f} – Neutral")
            score_pts += 0.5

        if "StochRSI_K" in df_ta.columns:
            st.markdown("**Stochastic RSI**")
            sk = float(df_ta["StochRSI_K"].iloc[-1])
            sd = float(df_ta["StochRSI_D"].iloc[-1])
            score_max += 1
            if sk > 80:
                st.error(f"StochRSI K={sk:.1f} – Overbought zone")
            elif sk < 20:
                st.success(f"StochRSI K={sk:.1f} – Oversold zone")
                score_pts += 1
            elif sk > sd:
                st.success(f"StochRSI K={sk:.1f} above D={sd:.1f} – Bullish crossover")
                score_pts += 0.75
            else:
                st.info(f"StochRSI K={sk:.1f} below D={sd:.1f} – Bearish crossover")
        ...
        score_max += 1
        if price_now > bb_u:
            st.error("Price above upper band – Overbought")
        elif price_now < bb_l:
        ...
        if "ATR_pct" in df_ta.columns:
            st.markdown("**ATR % (Volatility)**")
            atr_pct = float(df_ta["ATR_pct"].iloc[-1])
            atr_avg = float(df_ta["ATR_pct"].mean())
            if atr_pct > atr_avg * 1.5:
                st.warning(f"ATR {atr_pct:.2f}% – High volatility ({atr_pct/atr_avg:.1f}× avg)")
            elif atr_pct < atr_avg * 0.7:
                st.info(f"ATR {atr_pct:.2f}% – Low volatility – potential breakout building")
            else:
                st.info(f"ATR {atr_pct:.2f}% – Normal volatility ({atr_pct/atr_avg:.1f}× avg)")

        # Composite signal score bar
        if score_max > 0:
            st.markdown("---")
            st.markdown("**📊 Composite Signal Score**")
            pct_score = score_pts / score_max
            bar_color = "#3fb950" if pct_score >= 0.6 else ("#e67e22" if pct_score >= 0.4 else "#f85149")
            label = "Bullish" if pct_score >= 0.6 else ("Neutral" if pct_score >= 0.4 else "Bearish")
            st.markdown(
                f'''<div style="background:#21262d;border-radius:8px;padding:10px 14px;">
                  <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
                    <span style="color:#8b949e;font-size:0.8rem;">{label}</span>
                    <span style="color:{bar_color};font-weight:700;font-size:0.9rem;">{score_pts:.1f}/{score_max}</span>
                  </div>
                  <div style="background:#30363d;border-radius:4px;height:8px;">
                    <div style="width:{pct_score*100:.0f}%;background:{bar_color};border-radius:4px;height:8px;"></div>
                  </div>
